...
class ParsedownExtra extends Parsedown
{
	...
	protected function inlineFootnoteMarker($Excerpt)
	{
		...
		if (preg_match('/^\[\^(.+?)\]/', $Excerpt['text'], $matches))
		{
			...
			if ( ! isset($this->DefinitionData['Footnote'][$name]))
			{
				...
				// Added by Berean Archive to create a placeholder.
				$this->DefinitionData['Footnote'][$name] = array(
					'text' => 'placeholder3',
					'count' => 1,
					'number' => 1,
				);
				// end Added by Berean Archive

				//return; // commented out by Berean Archive
			}
	...
	{
		// Added by Berean Archive to fix footnotes being added to <div markdown="1">
		if ($this->tagDepth)
			return [];
		// End Added by Berean Archive.
			...
			if (($textElements[$n]['name']??'') === 'p')
			{
				$backLinkElements = array_merge(
					array(
						array(
							//'rawHtml' => '&#160;',
							'rawHtml' => ' ', // Modified by Beran Archive to not insert a space that causes a new line after a display: block element like <small> in the footnotes.
							'allowRawHtmlInSafeMode' => true,
						),
					),
					$backLinkElements
